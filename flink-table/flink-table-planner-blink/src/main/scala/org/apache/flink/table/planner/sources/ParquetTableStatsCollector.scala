...
import org.apache.flink.table.types.logical.{BigIntType, BooleanType, DateType, DecimalType, DoubleType, FloatType, IntType, LogicalType, TimeType, TimestampType, VarCharType}
...
object ParquetTableStatsCollector {
    ...
    val finalColumnStats = finalStatsMap.map {
      case (name, stats) =>
        ...
        val columnStats = new ColumnStats(
          null,
          if (stats.isEmpty) null else stats.getNumNulls,
          null,
          null,
          getMaxMinValueByTypeInfo(typeInfo, stats, isMax = true),
          getMaxMinValueByTypeInfo(typeInfo, stats, isMax = false)
        )
        (name, columnStats)
    }.toMap
  ...
  private def getMaxMinValueByTypeInfo(
    ...
    stat: Statistics[_],
    isMax: Boolean): Number = {
    typeInfo match {
      case _: BooleanType => stat match {
        case s: BooleanStatistics =>
//        TODO  if (isMax) s.getMax else s.getMin
          null
        case _ => null
      }
      case _: FloatType => stat match {
        case s: FloatStatistics => if (isMax) s.getMax else s.getMin
        case _ => null
      }
      case _: IntType => stat match {
        case s: IntStatistics => if (isMax) s.getMax else s.getMin
        case _ => null
      }
      case _: BigIntType => stat match {
        case s: LongStatistics => if (isMax) s.getMax else s.getMin
        case _ => null
      }
      case _: DoubleType => stat match {
      ...
      case _: VarCharType => stat match {
        case s: BinaryStatistics =>
          val v = if (isMax) s.genericGetMax else s.genericGetMin
          if (v != null) {
            null
//          TODO  v.toStringUsingUTF8
          } else {
            null
          }
        case _ => null
      }
      case _: DateType => stat match {
        case s: IntStatistics =>
          val v = if (isMax) s.getMax else s.getMin
          null
//        TODO  DateConverter.INSTANCE.toExternal(v)
        case _ => null
      }
      case _: TimeType => stat match {
        case s: IntStatistics =>
          val v = if (isMax) s.getMax else s.getMin
          null
//        TODO  TimeConverter.INSTANCE.toExternal(v)
        case _ => null
      }
      case _: TimestampType => stat match {
        case s: LongStatistics =>
          null
//         TODO new Timestamp(if (isMax) s.getMax else s.getMin)
        case s: BinaryStatistics =>
          val v = if (isMax) s.genericGetMax else s.genericGetMin
          if (v != null) {
            val nt = NanoTime.fromBinary(v)
            val ts = julianDayToTimestamp(nt.getJulianDay, nt.getTimeOfDayNanos)
            null
//          TODO  TimestampConverter.INSTANCE.toExternal(new Long(ts))
          } else {
            null
          }
        case _ => null
  ...
  /**
    ...
    */
  def julianDayToTimestamp(julianDay: Int, nanoseconds: Long): Long = {
    // use Long to avoid rounding errors
    val secondOfDay = (julianDay - DateTimeUtils.EPOCH_JULIAN) * 86400
    secondOfDay * (1000L * 1000L) + nanoseconds / 1000L
  }
  ...
  private def listFileStatus(
    ...
    nestedFileEnumeration: Boolean,
    fieldNames: Array[String],
    fieldTypes: Array[LogicalType]): util.List[FileStatus] = {
