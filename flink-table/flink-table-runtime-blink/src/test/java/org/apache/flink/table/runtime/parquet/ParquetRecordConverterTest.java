...
public class ParquetRecordConverterTest {
	...
	public void testSupportedConverters() {
		...
		LogicalType[] fieldTypes = {
				...
				new IntType(), // 6
				new IntType(), // 7
				new DecimalType(9, 2), // 8
				new BigIntType(), // 9
				new BigIntType(), // 10
				new DecimalType(9, 2), // 11
				DataTypes.BYTES().getLogicalType(), // 12
				DataTypes.STRING().getLogicalType(), // 13
				DataTypes.STRING().getLogicalType(), // 14
				DataTypes.STRING().getLogicalType(), // 15
				DataTypes.BYTES().getLogicalType(), // 16
				new DecimalType(9, 2), // 17
				new DateType(), // 18
				new TimeType(), //19
				new TimestampType(3) // 20
		};
		...
		assertEquals(SqlDateTimeUtils.internalToDate(100), row.getField(18));
		assertEquals(SqlDateTimeUtils.internalToTime(200), row.getField(19));
		assertEquals(SqlDateTimeUtils.internalToTimestamp(300), row.getField(20));
	}
